...
import GroupForms from "./groups/forms";
...
export default async function Home() {
  ...
  const [{ data: memberships }, { data: profile }] = await Promise.all([
    ...
    supabase.from("profiles").select("real_name").eq("id", user.id).single(),
  ]);
  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-[614px] px-4 py-5 pb-16">
        {approved.length > 0 && (
        ...
        {approved.length === 0 && pending.length === 0 && (
          <div className="mb-4 border border-line bg-card px-6 py-10 text-center">
            ...
            <p className="mt-3 text-sm text-muted">
              Join a group with a code, or start your own.
            </p>
          </div>
        )}
